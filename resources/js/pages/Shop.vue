        ...
        <main class="mx-auto max-w-[1280px] px-6 py-8">
            ...
            <div v-if="viewMode === 'browse'" class="space-y-12">
                ...
                <section class="space-y-6">
                    <div id="shop-collection-header" class="border-b border-neutral-200/80 pb-4 dark:border-neutral-800">
                        <h2 class="text-2xl font-bold tracking-tight">Shop the Collection</h2>
                        <p class="text-xs text-neutral-500">Filter and find the perfect premium item for your needs.</p>
                    </div>

                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                        <!-- Sidebar Filters (3 Cols on Desktop) -->
                        <aside class="lg:col-span-3 space-y-6 rounded-xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900 shadow-xs">
                            <div class="flex items-center justify-between">
                                <h3 class="font-bold text-sm tracking-tight flex items-center gap-2">
                                    <SlidersHorizontal class="h-4 w-4 text-emerald-500" />
                                    <span>Filters</span>
                                </h3>
                                <button 
                            ...
                            <!-- Search -->
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">Search Product</label>
                                <div class="relative">
                                    ...
                                    <input 
                                        ...
                                        class="w-full rounded-lg border border-neutral-200 pl-9 pr-4 py-2 text-xs focus:border-emerald-500 focus:outline-none dark:border-neutral-800 dark:bg-neutral-950 dark:text-white"
                                    />
                                </div>
                            </div>
                            ...
                            <!-- Categories List -->
                            <div class="space-y-2">
                                <label class="text-[10px] font-bold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">Category</label>
                                <div class="flex flex-col gap-1">
                                    <button 
                                        v-for="cat in categories" 
                                        :key="cat"
                                        @click="selectedCategory = cat"
                                        :class="selectedCategory === cat ? 'bg-emerald-50 text-emerald-600 font-semibold dark:bg-emerald-950/40 dark:text-emerald-400' : 'text-neutral-600 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-800/40'"
                            ...
                            <!-- Price Range -->
                            <div class="space-y-3">
                                <label class="text-[10px] font-bold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">Price Range</label>
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="space-y-1">
                                        <span class="text-[10px] text-neutral-400">Min ({{ $page.props.currency_symbol ?? '$' }})</span>
                                        <input 
                                    ...
                                    <div class="space-y-1">
                                        <span class="text-[10px] text-neutral-400">Max ({{ $page.props.currency_symbol ?? '$' }})</span>
                                        <input 
                                ...
                                <div class="flex flex-wrap gap-1.5 pt-1">
                                    <button 
                                        @click="minPrice = null; maxPrice = 100" 
                                        class="rounded bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1 text-[10px] font-medium text-neutral-600 dark:text-neutral-400 hover:bg-emerald-500 hover:text-white transition"
                                    >
                                        Under {{ $page.props.currency_symbol ?? '$' }}100
                                    </button>
                                    <button 
                                        @click="minPrice = 100; maxPrice = 250" 
                                        class="rounded bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1 text-[10px] font-medium text-neutral-600 dark:text-neutral-400 hover:bg-emerald-500 hover:text-white transition"
                                    >
                                        {{ $page.props.currency_symbol ?? '$' }}100 - {{ $page.props.currency_symbol ?? '$' }}250
                                    </button>
                                    <button 
                                        @click="minPrice = 250; maxPrice = null" 
                                        class="rounded bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1 text-[10px] font-medium text-neutral-600 dark:text-neutral-400 hover:bg-emerald-500 hover:text-white transition"
                                    >
                            ...
                            <div class="space-y-2 pt-3 border-t border-neutral-100 dark:border-neutral-800">
                                <label class="flex items-center gap-2 cursor-pointer select-none">
                                    <input 
                                        ...
                                        type="checkbox" 
                                        class="rounded border-neutral-300 text-emerald-600 focus:ring-emerald-500 h-4 w-4 dark:border-neutral-800 dark:bg-neutral-950"
                                    />
                                    <span class="text-xs font-semibold text-neutral-700 dark:text-neutral-300">In Stock Only</span>
                                </label>
                            </div>
                        </aside>
                        ...
                        <div class="lg:col-span-9 space-y-6">
                            ...
                            <div class="flex items-center justify-between gap-4 rounded-xl border border-neutral-100 bg-neutral-50 px-4 py-3 dark:border-neutral-800/60 dark:bg-neutral-900/40">
                                <span class="text-xs text-neutral-500 dark:text-neutral-400">
                                    Showing <span class="font-bold text-neutral-800 dark:text-white">{{ filteredProducts.length }}</span> products
                                </span>
                                
                                <div class="flex items-center gap-2">
                                    <span class="text-xs text-neutral-400">Sort by:</span>
                                    <select 
                                        v-model="sortBy"
                            ...
                            <div v-if="filteredProducts.length > 0" class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                                ...
                                >
                                    ...
                                    <span 
                                        ...
                                        class="absolute top-3 left-3 z-10 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider"
                                    >
                                        {{ product.badge }}
                                    </span>
                                    ...
                                    >
                                        ...
                                        />
                                        <div v-else :class="product.imageGradient" class="absolute inset-0 bg-gradient-to-tr opacity-90 transition-transform duration-500 group-hover:scale-105"></div>
                                    ...
                                    <div class="flex flex-col flex-1 p-5 space-y-3">
                                        <div class="space-y-1">
                                            <span class="text-[10px] font-semibold text-emerald-500 uppercase tracking-wider">{{ product.category }}</span>
                                            <h3 
                                                class="line-clamp-2 h-10 text-sm font-semibold leading-tight text-neutral-900 hover:text-emerald-500 cursor-pointer transition dark:text-white"
                                                @click="selectedProduct = product"
                                            >
                                                {{ product.name }}
                                            </h3>
                                        ...
                                        <div class="flex items-center justify-between pt-2">
                                            <div class="flex flex-col">
                                                <span class="font-mono text-base font-bold text-neutral-900 dark:text-white">
                                                    {{ $page.props.currency_symbol ?? '$' }}{{ product.price.toFixed(2) }}
                                                </span>
                                                <span v-if="product.originalPrice" class="font-mono text-xs text-neutral-400 line-through">
                                                    {{ $page.props.currency_symbol ?? '$' }}{{ product.originalPrice.toFixed(2) }}
                                                </span>
                                            </div>
                                            ...
                                            <button 
                                                ...
                                                class="rounded-lg bg-neutral-900 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-600 disabled:bg-neutral-200 disabled:text-neutral-500 transition dark:bg-neutral-800 dark:hover:bg-emerald-600 dark:disabled:bg-neutral-800"
                                            >
                            ...
                            <div v-else class="flex flex-col items-center justify-center py-20 text-center space-y-4 rounded-xl border border-dashed border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900">
                                <div class="rounded-full bg-neutral-100 p-4 dark:bg-neutral-950">
